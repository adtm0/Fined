...
def dashboard(request):
  ...
  cal_year = int(request.GET.get('year', date.today()))
  cal = calendar.Calendar(firstweekday=6)
  calendar_weeks = cal.monthdayscalendar(date.today().year, date.today().month)
  calendar_month_name = date.today().strftime('%B %Y')
