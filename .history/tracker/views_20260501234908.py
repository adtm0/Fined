...
def dashboard(request):
  ...
  total_balance = sum(account.balance for account in wallets)
...
def add_transaction(request, wallet_id):
  account = Wallet.objects.get(id=wallet_id, user=request.user)
  if request.method == 'POST':
...
def wallet_detail(request, wallet_id):
  ...
  return render(request, 'tracker/wallet_detail.html', {'account': account, 'transactions': transactions})
  ...
  if request.method == 'POST':
    ...
    if form.is_valid():
      ...
      if transaction.type == 'income':
        ...
        account.balance -= transaction.amount
      account.save()
      return redirect('wallet_detail', wallet_id=account.id)
  else:
...
def edit_wallet(request, wallet_id):
  account = Wallet.objects.get(id=wallet_id, user=request.user)
  if request.method =='POST':
    form = WalletForm(request.POST, instance=account)
    if form.is_valid():
      form.save()
      return redirect('dashboard')
  else:
    form = WalletForm(instance=account)
  return render(request, 'tracker/edit_wallet.html', {'form': form, 'account': account})
